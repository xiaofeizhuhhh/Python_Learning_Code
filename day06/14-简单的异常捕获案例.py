...
def use_exception1():
    try:
        num = int(input('请输入一个整数：'))  # 将读取到的输入（均为字符串）转换为整数
        print(num)
    except:
        print('输入的必须是整型数字')
...
def use_exception3():
    ...
    try:
        ...
        print("请输入正确的整数")
    except Exception as e:  # e代表异常对象的别名,如 ValueError,ZeroDivisionError等
        print(e)
...
def use_exception4():
    ...
    try:
        ...
        print("正常执行")  # 有return、break、continue语句时，else不会执行
    finally:
